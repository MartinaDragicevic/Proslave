...
public class OwnerDashboard {
    ...
    public void initialize() {
        ...
        Database.owners.stream()
                .filter(owner -> owner.getKorisnickoIme().equals(loggedInUser))
                .findFirst()
                .ifPresent(owner -> {
    ...
    private void changePassword() {
        ...
        try {
            ...
            showMessage("Error changing password: " +
                    (e.getMessage() != null ? e.getMessage() : "Unknown error"), Color.RED, 4);
            e.printStackTrace();
        }
    }
    ...
    private void showMessage(String message, Color color, double seconds) {
        ...
        delay.setOnFinished(evt -> errorLabel.setText(""));
        delay.play();
    }
    ...
    public void addNewVenue(ActionEvent event) throws IOException {
        switchScene(event, "owner_newVenue.fxml");
    }
    public void venueList(ActionEvent event) throws IOException {
        switchScene(event, "owner_venueList.fxml");
    }
